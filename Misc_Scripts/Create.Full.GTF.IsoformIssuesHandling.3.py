...
#this script in particular will examine 30 isoforms that have amino acids matches passed the 6th amino acid
#will also pull gene category as this will correct all isoform positions
#to run script: python3 Create.Full.GTF.IsoformIssuesHandling.1.py <source as string; in this case all sources will be PacBio> <classification file for combined analyses> < bed file with only 30 isoforms> <full Ensembl GTF> <isoseq gtf with exon positions> <isoseq fasta file> <isoseq faa file> <output file>
...
def write():
    ...
    with open(output, 'a') as out:
        for iso in start_codon_features:
            for i in start_codon_features[iso]:
                final = "\t".join(i)
                out.write(final + "\n")
        for iso2 in stop_codon_features:
            for i in stop_codon_features[iso2]:
                final = "\t".join(i)
                out.write(final + "\n")
        for iso3 in five_prime_utr_features:
            for i in five_prime_utr_features[iso3]:
                final = "\t".join(i)
                out.write(final + "\n")
        for iso4 in three_prime_utr_features:
            for i in three_prime_utr_features[iso4]:
                final = "\t".join(i)
                out.write(final + "\n")
        for iso5 in cds_features:
            for i in cds_features[iso5]:
                final = "\t".join(i)
                out.write(final + "\n")

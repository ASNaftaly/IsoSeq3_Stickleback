...
#this script handles the 1,659 isoforms that have the correct minus strand stop position but have an ATG as start position/sequence
#to run script: python3 Create.Full.GTF.IsoformIssuesHandling.6.py <source as string; in this case all sources will be PacBio> <classification file for combined analyses> <full bed file> <full Ensembl GTF> <isoseq gtf with exon positions> <isoseq fasta file> <isoseq faa file> <isoforms to examine; file contains just isoform ids> <output file>
...
def convert_start_positions():
    ...
    count = 0
    del start_codons["PB.13809.4"]
    del start_codons["PB.13809.5"]
            ...
            #will probably want to specify this in the final gtf file
                ...
                while x < len(exon_length_list):
                    if moving_start_codon - exon_length_list[x] > 0:
                        ...
                        converted_start_codons.update({isoform:new_start_codon_pos})
                        break
    #manually convert 2 isoforms:
    converted_start_codons.update({"PB.14344.9":["3113778", "3113776"]})
    converted_start_codons.update({"PB.13809.1":["3843240", "3843238"]})
...
def convert_stop_positions():
    ...
    del stop_codons["PB.13809.4"]
    del stop_codons["PB.13809.5"]
                ...
                if exon_1_length > single_codon_start:
                    ...
                    while x < len(exon_length_list):
                        if moving_stop_codon - exon_length_list[x] > 0:
                            ...
                            converted_stop_codons.update({isoform:new_stop_codon_pos})
                            break
    converted_stop_codons.update({"PB.14344.9":["3120678", "3120676"]})
    converted_stop_codons.update({"PB.13809.1":["3847104", "3847102"]})
